...
;;         )))
(defun my-notes-capture-template ()
  (format "#+TITLE: %%?\n#+DATE: %s\n#+CATEGORY: %s\n#+LAYOUT: post\n" (format-time-string "%Y-%m-%d %H:%M:%S") (plist-get org-capture-plist :category)))
(setq org-capture-templates
      '(("t" "inbox" entry (file+headline "~/Dropbox/org/inbox.org" "inbox")
         "** TODO %?\n   %i\n  %t\n")
        ("c" "code" plain (file (format "~/notes/_posts/%s_code.org" (format-time-string "%Y-%m-%d-%H%M%S")) "code")
         (function my-notes-capture-template)
         :category "code")
        ("e" "event" plain (file (format "~/notes/_posts/%s_event.org" (format-time-string "%Y-%m-%d-%H%M%S")) "event")
         (function my-notes-capture-template)
         :category "event")
        ("h" "health" plain (file (format "~/notes/_posts/%s_health.org" (format-time-string "%Y-%m-%d-%H%M%S")) "health")
         (function my-notes-capture-template)
         :category "health")
        ("i" "item" plain (file (format "~/notes/_posts/%s_item.org" (format-time-string "%Y-%m-%d-%H%M%S")) "item")
         (function my-notes-capture-template)
         :category "item")
        ("b" "book" plain (file (format "~/notes/_posts/%s_book.org" (format-time-string "%Y-%m-%d-%H%M%S")) "book")
         (function my-notes-capture-template)
         :category "book")
